...
impl From<horizon_core::schema::magnitude::NotaDecodeError> for NotaDecodeError {
    fn from(error: horizon_core::schema::magnitude::NotaDecodeError) -> Self {
...
pub struct ServiceName(pub Text);

#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct CacheUrl(pub Text);

#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct RejectionReason(pub Text);
...
#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ClusterTrust(pub Magnitude);
...
#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct BinaryCache(pub CacheUrl);
...
#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ProjectionError(pub RejectionReason);

#[derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize, Clone, Debug, PartialEq, Eq)]
pub enum Input {
    Project(ClusterProposal),
}
    ...
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        if let Some(variant) = block.demote_to_string() {
            return match variant {
                ...
                other => Err(NotaDecodeError::UnknownVariant { enum_name: "NodeRole", variant: other.to_owned() }),
            };
        }
        Err(NotaDecodeError::ExpectedAtom { type_name: "NodeRole" })
    }
...
impl ClusterTrust {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        Ok(Self(Magnitude::from_nota_block(block)?))
    }
...
impl NodeProposal {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        let children = NotaBlock::new(block).expect_children(nota_next::Delimiter::Parenthesis, "parenthesis", "NodeProposal", 3)?;
        Ok(Self {
            role: NodeRole::from_nota_block(&children[0])?,
            trust: Magnitude::from_nota_block(&children[1])?,
            services: NotaCollection::new(&children[2]).parse_vector(ServiceName::from_nota_block)?,
        })
    }
    ...
    pub fn to_nota(&self) -> String {
        let fields = [
            self.role.to_nota(),
            self.trust.to_nota(),
            NotaCollection::format_vector(&self.services, ServiceName::to_nota),
        ];
        format!("({})", fields.join(" "))
    }
}
...
impl BinaryCache {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        Ok(Self(CacheUrl::from_nota_block(block)?))
    }
...
impl ClusterProposal {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        let children = NotaBlock::new(block).expect_children(nota_next::Delimiter::Parenthesis, "parenthesis", "ClusterProposal", 4)?;
        Ok(Self {
            nodes: NotaCollection::new(&children[0]).parse_map(NodeName::from_nota_block, NodeProposal::from_nota_block)?,
            trust: ClusterTrust::from_nota_block(&children[1])?,
            cache: NotaCollection::new(&children[2]).parse_option(BinaryCache::from_nota_block)?,
            cluster_services: NotaCollection::new(&children[3]).parse_vector(ServiceName::from_nota_block)?,
        })
    }

    pub fn to_nota(&self) -> String {
        let fields = [
            NotaCollection::format_map(&self.nodes, NodeName::to_nota, NodeProposal::to_nota),
            self.trust.to_nota(),
            NotaCollection::format_option(&self.cache, BinaryCache::to_nota),
            NotaCollection::format_vector(&self.cluster_services, ServiceName::to_nota),
        ];
        format!("({})", fields.join(" "))
    }
}
...
impl NodeConfig {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        let children = NotaBlock::new(block).expect_children(nota_next::Delimiter::Parenthesis, "parenthesis", "NodeConfig", 4)?;
        Ok(Self {
            role: NodeRole::from_nota_block(&children[0])?,
            trust: Magnitude::from_nota_block(&children[1])?,
            services: NotaCollection::new(&children[2]).parse_vector(ServiceName::from_nota_block)?,
            cache: NotaCollection::new(&children[3]).parse_option(CacheUrl::from_nota_block)?,
        })
    }
    ...
    pub fn to_nota(&self) -> String {
        let fields = [
            ...
            self.trust.to_nota(),
            NotaCollection::format_vector(&self.services, ServiceName::to_nota),
            NotaCollection::format_option(&self.cache, CacheUrl::to_nota),
        ];
        format!("({})", fields.join(" "))
    }
}
...
impl Input {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        ...
        match variant {
            "Project" => Ok(Self::Project(ClusterProposal::from_nota_block(&children[1])?)),
...
impl Output {
    pub fn from_nota_block(block: &nota_next::Block) -> Result<Self, NotaDecodeError> {
        ...
        match variant {
            ...
            "Rejected" => Ok(Self::Rejected(ProjectionError::from_nota_block(&children[1])?)),
...
pub trait InputNexus {
    ...
    fn project(&self, mail: NexusMail<ClusterProposal>) -> Result<Self::Reply, Self::Error>;
}
...
impl Input {
        ...
        Nexus: InputNexus,
    {
        let reply = match self {
            Self::Project(payload) => nexus.project(NexusMail::new(identifier, payload)),
        }?;
...
pub trait OutputNexus {
    ...
    fn rejected(&self, mail: NexusMail<ProjectionError>) -> Result<Self::Reply, Self::Error>;
}
...
impl Output {
    ...
    {
        let reply = match self {
            Self::Projected(payload) => nexus.projected(NexusMail::new(identifier, payload)),
